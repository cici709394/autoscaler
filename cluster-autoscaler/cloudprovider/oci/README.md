...
- **Ephemeral-storage for OKE node pools scaling from zero**: when a node pool has no running nodes,
  ...
  1. The freeform tag `cluster-autoscaler/node-ephemeral-storage` on the node pool, if set to a
     valid parseable value (e.g. `40Gi`). A valid tag takes priority.
  2. Otherwise, if the tag is missing or invalid, the boot-volume size configured in
     `NodeSourceDetails.BootVolumeSizeInGBs` is used when available. Note that this is the **raw
     disk size**, not the kubelet allocatable value — actual allocatable ephemeral-storage will be
     somewhat lower due to OS/rootfs usage, kube-reserved, system-reserved, and eviction headroom.
     Use a valid explicit freeform tag for a tighter bound if needed.
